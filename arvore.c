...
NoArv* remover(NoArv *raiz, int chave) {
    ...
    if (raiz->valor == chave) {
        if (raiz->esquerda == NULL && raiz->direita == NULL) {
            ...
            raiz->esquerda = remover(raiz->esquerda, pontaux->valor);
        } else {
...
void exibir_pre_ordem(NoArv *raiz) {
    if (raiz) {
        printf("%d ", raiz->valor);
...
void exibir_in_ordem(NoArv *raiz) {
    if (raiz) {
...
void exibir_pos_ordem(NoArv *raiz) {
    if (raiz) {
...
int main() {
    ...
    // Limpeza da árvore antes de sair
    esvaziar(&raiz);
